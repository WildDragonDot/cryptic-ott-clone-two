...
if(isset($_POST['add_video_uuid'])){
    ...
    $result = mysqli_query($con, "SELECT `video_view` FROM `video_info` WHERE `video_uuid` = '$add_video_uuid' AND `module_uuid` = '$add_module_uuid'");    
    if (mysqli_num_rows($result) > 0 ) {
        ...
        while ($row = mysqli_fetch_array($result)) {
                ...
                $id = $id + 1;
        $query = "UPDATE `video_info` SET `video_view` = '$id' WHERE `video_uuid` = '$add_video_uuid' AND `module_uuid` = '$add_module_uuid'";
